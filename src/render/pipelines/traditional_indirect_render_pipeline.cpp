...
#include "traditional_indirect_render_pipeline.h"
...
namespace Renderer
{
TraditionalIndirectRenderPipeline::TraditionalIndirectRenderPipeline() = default;

TraditionalIndirectRenderPipeline::~TraditionalIndirectRenderPipeline() = default;

TraditionalIndirectRenderPipeline::TraditionalIndirectRenderPipeline(VulkanContext* context) : context(context)
{
    VkPushConstantRange renderPushConstantRange{};
    renderPushConstantRange.offset = 0;
    renderPushConstantRange.size = sizeof(TraditionalIndirectRenderPushConstant);
    ...
    if (!VkHelpers::LoadShaderModule("shaders\\indirectTraditional_vertex.spv", context->device, &vertShader)) {
        throw std::runtime_error("Error when building the vertex shader (indirectDraw_vertex.spv)");
    }
    if (!VkHelpers::LoadShaderModule("shaders\\indirectTraditional_fragment.spv", context->device, &fragShader)) {
...
TraditionalIndirectRenderPipeline::TraditionalIndirectRenderPipeline(TraditionalIndirectRenderPipeline&& other) noexcept
{
...
TraditionalIndirectRenderPipeline& TraditionalIndirectRenderPipeline::operator=(TraditionalIndirectRenderPipeline&& other) noexcept
{
    if (this != &other) {
        pipelineLayout = std::move(other.pipelineLayout);
        pipeline = std::move(other.pipeline);
