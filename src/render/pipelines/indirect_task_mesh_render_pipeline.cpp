...
#include "indirect_task_mesh_render_pipeline.h"
...
namespace Renderer
{
IndirectTaskMeshRenderPipeline::IndirectTaskMeshRenderPipeline() = default;

IndirectTaskMeshRenderPipeline::~IndirectTaskMeshRenderPipeline() = default;

IndirectTaskMeshRenderPipeline::IndirectTaskMeshRenderPipeline(VulkanContext* context) : context(context)
{
    VkPushConstantRange renderPushConstantRange{};
    renderPushConstantRange.offset = 0;
    renderPushConstantRange.size = sizeof(IndirectTaskMeshRenderPushConstant);
    ...
    if (!VkHelpers::LoadShaderModule("shaders\\indirectMeshTask_task.spv", context->device, &taskShader)) {
        throw std::runtime_error("Error when building the vertex shader (meshShader_task.spv)");
    }
    if (!VkHelpers::LoadShaderModule("shaders\\indirectMeshTask_mesh.spv", context->device, &meshShader)) {
        throw std::runtime_error("Error when building the fragment shader (meshShader_mesh.spv)");
    }
    if (!VkHelpers::LoadShaderModule("shaders\\indirectMeshTask_fragment.spv", context->device, &fragmentShader)) {
...
IndirectTaskMeshRenderPipeline::IndirectTaskMeshRenderPipeline(IndirectTaskMeshRenderPipeline&& other) noexcept
{
...
IndirectTaskMeshRenderPipeline& IndirectTaskMeshRenderPipeline::operator=(IndirectTaskMeshRenderPipeline&& other) noexcept
{
    if (this != &other) {
        pipelineLayout = std::move(other.pipelineLayout);
        pipeline = std::move(other.pipeline);
