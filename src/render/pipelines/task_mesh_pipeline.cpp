...
TaskMeshPipeline::TaskMeshPipeline(VulkanContext* context) : context(context)
{
    ...
    renderPushConstantRange.size = sizeof(TaskMeshPushConstant);
    ...
    if (!VkHelpers::LoadShaderModule("shaders\\meshTask_task.spv", context->device, &taskShader)) {
        throw std::runtime_error("Error when building the vertex shader (meshShader_task.spv)");
    }
    if (!VkHelpers::LoadShaderModule("shaders\\meshTask_mesh.spv", context->device, &meshShader)) {
        throw std::runtime_error("Error when building the fragment shader (meshShader_mesh.spv)");
    }
    if (!VkHelpers::LoadShaderModule("shaders\\meshTask_fragment.spv", context->device, &fragmentShader)) {
