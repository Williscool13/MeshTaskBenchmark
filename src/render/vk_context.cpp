...
VulkanContext::VulkanContext(SDL_Window* window)
{
    ...
    if (res != VK_SUCCESS) {
        ...
        fmt::println("Failed to initialize volk: {}", string_VkResult(res));
        exit(1);
    }
    ...
    auto resultInstance = builder.set_app_name("Mesh Task Benchmark")
    ...
    // Queues and queue family
    {
        ...
        if (!graphicsQueueResult) {
            fmt::println("Failed to get graphics queue: {}", graphicsQueueResult.error().message());
            exit(1);
        }
        ...
        if (!graphicsQueueFamilyResult) {
            fmt::println("Failed to get graphics queue family index: {}", graphicsQueueFamilyResult.error().message());
            exit(1);
        }
        ...
        if (!transferQueueResult) {
            fmt::println("Failed to get transfer queue: {}", transferQueueResult.error().message());
            exit(1);
        }
        ...
        if (!transferQueueFamilyResult) {
            fmt::println("Failed to get transfer queue family index: {}", transferQueueFamilyResult.error().message());
            exit(1);
        }
    ...
    fmt::println("Transfer Queue:        0x{:016x}", reinterpret_cast<uintptr_t>(transferQueue));
    fmt::println("Transfer Queue Family: {}", transferQueueFamily);
